...
> 来源: [`plans/README.md`](plans/README.md)（版本路线图）、各版本详细计划文件（`plans/0.9.7.md` ~ `plans/1.1.0-dev.1.md`）。
...
```
0.9.0 → 0.9.1 → 0.9.2 → 0.9.3 → 0.9.4 → 0.9.5 → 0.9.5.1 → 0.9.6 → 0.9.7 ✅ → 0.9.8 ✅ → 0.9.9 ✅ → 0.9.10 (current) → 1.0.0
发布    默认仓库  文档     捆绑包   文档与   跨平台    代码重构    功能补全    仓库扩展          CLI改进    安装钩子  代码质量  正式版
正式版  创建      多语言   迁移     质量完善  体验与    与质量清理  与生态完善  与扩充            与扩充     Lua化    重构      发布
                                                质量保障
```

已完成 21 个版本（0.1.6 ~ 0.9.9），当前正在执行 0.9.10（代码质量重构），剩余 1 个版本后到达 1.0.0。
...
## 三、已完成版本：0.9.8 — CLI 改进、默认仓库扩充与文档检查 ✅

> 详细设计: [`plans/0.9.8.md`](plans/0.9.8.md) · 提交: `c916d8b`
...
- [x] 改造 `pkg::info()` 所有 `std::cout` → `info_line()`
- [x] 改造 `repo::info()` / `repo::list()` 所有 `std::cout` → `info_line()`
- [x] 审查 i18n 键值是否多余前缀，必要时去除（确认无需修改）

**阶段二：简写展开提示** ✅
- [x] `CliArgs` 新增 `shorthand_expansion` 字段
- [x] 简写展开时填充字段 → `main.cpp` 在 verbose 时输出
- [x] 新增 i18n 键 `shorthand_expansion`（en/zh）
- [x] `-v`/`--verbose` 全局化：所有子命令接受该标志（静默忽略），pre-scan 尊重 `--` 边界

**阶段三：Header-Only 包支持 + 默认仓库扩充** ✅
- [x] `pkg.toml` 解析 `header_only = true` → 跳过编译步骤（0.9.7 已实现）
- [x] `ezmk pkg info` 适配（标注 `Type: header-only`）
- [x] 制作 10 个 stb 包 + 10 个 Boost header-only 子库
- [x] 每个包通过头文件完整性验证

**阶段四：校验** ✅
- [x] `build.sh` 编译通过 + 全量测试通过（524 用例 / 2411 断言）
- [x] CLI 输出格式手动验证（`[ezmk]` 前缀 + 简写展开提示）
- [x] 新包在默认仓库中的 `index.toml` 一致性校验（validate.sh 4/4 通过）

---

## 四、已完成版本：0.9.9 — 安装钩子 Lua 化 ✅
...
**阶段一：核心实现** ✅
- [x] `detect_install_script()` 新增 `.lua` 优先级检测
- [x] `run_install_script()` 新增 `is_lua` 分支
- [x] 新增 `lua::run_install_hook_script()`（`src/lua_api.cpp` + `include/ezmk/lua_api.hpp`）
- [x] `install()` 函数传参适配

**阶段二：测试** ✅
- [x] 创建 9 个测试用例（基本执行、ctx 表、退出码、空指针、缺失 run()、Lua error、语法错误、scope、无配置降级）
- [x] 测试正常执行 / 失败处理 / Lua error 展示 / 优先级检测 / 向后兼容

**阶段三：文档** ✅
- [x] 更新 `docs/en/pkg.md` + `docs/zh/pkg.md`（安装钩子章节新增 Lua 钩子说明、ctx 表、示例代码）
- [x] 更新安全模型文档（`docs/en/@safety.md` + `docs/zh/@safety.md`）

**阶段四：校验** ✅
- [x] `build.sh` 编译通过 + 全量测试通过（533 用例 / 2430 断言，零回归）
- [x] Windows 手动验证 Lua 安装钩子端到端流程
...
## 五、当前版本：0.9.10 — 代码质量重构（执行计划）

> 详细设计: [`plans/0.9.10.md`](plans/0.9.10.md)

### 5.1 背景

0.9.9 完成了安装钩子 Lua 化，但代码审查发现 4 项实现层面的技术债务：`run_install_hook_script` 与 `run_hook_script` 约 70 行重复代码；`run_install_script()` 参数膨胀至 9 个；Lua 栈状态隐式契约；`detect_install_script()` 因 static 不可测。
...
| # | 交付物 | 说明 |
|---|--------|------|
| 1 | **提取通用 sandbox 框架** | 新增 `run_lua_script_with_ctx()`，消除 ~70 行重复；`run_hook_script` 和 `run_install_hook_script` 退化为薄封装 |
| 2 | **压缩参数** | 引入 `InstallHookContext` 结构体，`run_install_script()` 参数 9→6 |
| 3 | **Lua 栈断言** | `register_api()` 调用前后增加 `lua_gettop` 断言，防止栈泄漏 |
| 4 | **可测试化** | `detect_install_script()` 移至 `pkg.hpp` + 3 个单元测试 |

### 5.3 执行阶段

**阶段一：通用 sandbox 框架**
- [ ] 定义 `BuildCtxFn` 类型 + `run_lua_script_with_ctx()` 实现
- [ ] `run_hook_script()` → 薄封装
- [ ] `run_install_hook_script()` → 薄封装
- [ ] 通用函数中加入栈位置断言

**阶段二：参数压缩**
- [ ] 定义 `InstallHookContext` 结构体
- [ ] `run_install_script()` 签名更新 + 调用点适配

**阶段三：可测试化**
- [ ] `detect_install_script()` 移入 `pkg.hpp`
...
| 3 | Git tag 覆盖范围 — 缺失 v0.9.3 ~ v0.9.10 | 高 |
...
## 七、后续展望：1.1.0 — MSVC 包编译、确定性构建与产物安装

> 详细设计: [`plans/1.1.0-dev.1.md`](plans/1.1.0-dev.1.md)（规划中，尚未纳入路线图）
...
> 注：1.1.0 尚在规划阶段，未正式列入路线图。计划文件 `plans/1.1.0-dev.1.md` 待 1.0.0 发布后启用。
...
```
0.1.6 ────── 0.2.6 ────── 0.9.0 ──── 0.9.5 ──── 0.9.5.1 ──── 0.9.6 ──── 0.9.7 ──── 0.9.8 ──── 0.9.9 ──── 0.9.10 ──── 1.0.0
测试框架搭建   CLI/翻译收尾   发布准备     集成测试     代码清理      功能补全    生态扩展    CLI与生态   技术债务清零   代码质量重构   正式发布
(已完成)       (已完成)       (已完成)     (已完成)     ✅ 已完成     ✅ 已完成   ✅ 已完成   ✅ 已完成   ✅ 已完成   ← 当前 →       终点
```
...
- 单元测试：533 用例 / 2430 断言（0.9.9 全量测试通过）
...
1. ✅ **CLI 前缀补全** — `util::info_line()` 函数 + `pkg::info()` / `repo::info()` / `repo::list()` 输出统一 `[ezmk]` 前缀
2. ✅ **简写展开提示** — `CliArgs::shorthand_expansion` 字段 + `--verbose` 时显示简写 → 全名映射
3. ✅ **Header-Only 包支持 + 默认仓库扩充** — 10 个 stb 单文件库 + 10 个 Boost header-only 子库（共 20 个新包），`pkg info` 适配 `Type: header-only`
4. ✅ **校验** — `build.sh` 编译通过 + 全量测试通过（524 用例 / 2411 断言），`validate.sh` 4/4 通过

> 下一步 → **0.9.9**: 安装钩子 Lua 化，全项目统一技术栈 ✅ 已完成
...
1. ✅ **核心实现** — `detect_install_script()` `.lua` 优先检测 + `run_install_script()` Lua 分支 + `run_install_hook_script()`（`src/lua_api.cpp` + `include/ezmk/lua_api.hpp`）+ `install()` 传参适配
2. ✅ **测试** — 9 个测试用例：基本执行、ctx 表完整性（6 字段）、退出码、空指针、缺失 run()、Lua error、语法错误、scope 值、无配置降级
3. ✅ **文档** — `docs/en/pkg.md` + `docs/zh/pkg.md`（Lua 钩子规范、ctx 表、示例代码）+ `docs/en/@safety.md` + `docs/zh/@safety.md`（安装钩子安全说明）
4. ✅ **校验** — `build.sh` 编译通过 + 全量测试通过（533 用例 / 2430 断言），零回归

> 下一步 → **0.9.10**: 代码质量重构 ← 当前执行
