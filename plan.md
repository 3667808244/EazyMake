...
> **✅ 状态：已发布**（2026-08-10，tag `v1.1.3`）。下一版本：**1.2.0**（`plans/1.2.0/`）。
...
- [x] **发布门槛预检**：① 计划清单全部完成或明确收口（含 S1b 决策）；② 公共 API 无破坏性变更（均增量/内部）；③ 全量测试零回归（630 用例 / 2918 断言）
- [x] 打 `v1.1.3` tag 触发 `release.yml`；产物核验 + 分发（沿用 1.1.2 流程）— **2026-08-10 已发布**（gh release + release.yml 构建 win/linux/macOS 产物）
- [x] 收尾：`plan.md` 状态更新（1.1.3 已发布 → 恢复 1.2.0）；`plans/README.md` / `plans/1.1.x/README.md` 状态更新 — **已完成**
