...
> **状态：规划中**（2026-08-31 立项）。1.4.x 系列路线图见 [`plans/1.4.x/README.md`](plans/1.4.x/README.md)。
>
...
- [ ] **1.1** `util::is_git_url()` 提取（**保留 repo 宽松语义**：`git@`/任意 `://`/含点含斜杠 + 本地路径排除，repo add 行为零变化），`repo.cpp` 改调 util 版本（repo add 兼容性回归测试覆盖）；`pkg.cpp` 用严格子集判定（剥 `#ref` 后 `.git` 结尾 / `git@` / `git://` / `file://`）
- [ ] **1.2** `pkg.cpp` 检测顺序插入 git 分支：目录 → **git** → 归档 URL → 名称搜索；判定短路不误伤 `.zip`/`.tar.gz`
- [ ] **1.3** 克隆 → 临时目录 → `install_from_directory` 全链路；git 不可用 → fatal；`util::git_head_commit()` helper（`rev-parse HEAD`）
- [ ] **1.4** 单测：`is_git_install_source` 判定表（命中：`git@`/`git://`/`file://`/`.git`/`.git#ref`；不误判：`.zip`/`.tar.gz`/`foo.zip#x`/无协议非 git）

### 阶段二：ref 定位与克隆策略（对应设计 §3.2）

- [ ] **2.1** `#<ref>` 片段解析（最后一个 `#` 后全部，克隆前剥离）+ `--branch <ref>` flag（优先级：flag > 片段 > 默认分支）
- [ ] **2.2** `util::git_clone` 加 `shallow` 参数（默认 false，repo 调用零变化）：分支/标签/默认分支 `--depth 1`；commit SHA 全量克隆 + checkout
- [ ] **2.3** 集成测试：`#tag` / `#<sha>` / `--branch` / 默认分支四形态安装，lockfile commit 与 `rev-parse HEAD` 一致

### 阶段三：lockfile commit + `--locked` 校验（对应设计 §3.3）

- [ ] **3.1** `LockfilePackage` 新增可选 `commit` 字段（load/save 兼容：空串缺省、非空才写出、lockfile version 保持 1）
- [ ] **3.2** git 源写入 `source="git"` + `source_url` + `commit`；sha256 留空
- [ ] **3.3** `--locked`：按 lockfile 的 source_url+commit 克隆，`rev-parse HEAD` 不一致 → fatal（`lock_commit_mismatch`）；lockfile 不重写
- [ ] **3.4** 测试：commit 匹配成功 / 篡改后失败 / 旧 lockfile（无 commit 字段）正常解析

### 阶段四：i18n 三向（对应设计 §3.4）

- [ ] **4.1** 5 个新 key（`pkg_git_cloning` / `pkg_git_not_available` / `pkg_git_sha256_skipped` / `pkg_git_plain_confirm` / `lock_commit_mismatch`）入 `i18n_keys.def` + en/zh JSON（zh-TW 继承回退）
- [ ] **4.2** `python scripts/check_i18n.py` 通过（键数 397 → 402）

### 阶段五：测试补齐 + 收口（对应设计 §3.6/§4）

- [ ] **5.1** 集成补齐：`file://` 端到端（fixture `git init` + tag + 提交）、`--sha256` 跳过提示、`git://` 明文确认（`-y` 跳过）
- [ ] **5.2** 全量回归 `bash build.sh test-all` 零失败（**1003 用例 / 5835 断言**基线，零回归）；CI 工作流覆盖核对
- [ ] **5.3** 文档：docs/en|zh `pkg.md`「Package Sources」新增 Git 小节 + 检测顺序、`cli.md` `pkg install` 参数表（git URL / `--branch`）、README 速览核对、`.claude/skills/ezmk-user-pkg/SKILL.md` 同步
- [ ] **5.4** `CHANGES.md` 1.4.1 条目（新增 / 行为变更 / 测试 / 已知限制）
- [ ] **5.5** `plans/1.4.x/README.md` / `plan.md` / `plans/README.md` 状态更新（全勾选）
- [ ] **5.6** 发布门槛复核（⛔：① 清单全部完成或明确收口 ② 公共 API 无破坏性变更 ③ 全量零回归）——满足后按 workflow 正式发布（bump 1.4.1）
