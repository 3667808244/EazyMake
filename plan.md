...
- [x] **2.1 `Command::Workspace*` + cli 解析**（list/build/test/clean/--member/--stop-on-error/-j）+ main 分发 + `-w` 重定向 + help 文本
- [x] **2.2 Kahn 拓扑分层**：`topo_layers(ws)`（依赖层先构建，同层并行）
- [x] **2.3 子进程执行器**（`src/workspace_build.cpp`）：每成员子进程 + 层内 ThreadPool + 输出成员前缀 + 结果汇总（成功/失败/耗时/skipped）+ 退出码；`--stop-on-error` = 失败后停派发（本层未启动 + 后续层 skipped）、不 kill 在跑成员
- [x] **2.4 兄弟产物注入 = 成员自发现**：build.cpp 读 `[depends] workspace` → locate/load（复用 dev.1）→ 拼 `-I/-L/-l`（存在性门控）；**零环境变量**（坑 1）；签名含注入参数
- [x] **2.5 命令行长度兜底**：编译/链接命令 >16K → GCC 响应文件 `@<rsp>`（坑 1 兜底）
- [x] **2.6 跨成员增量**：库 `.cpp` → 依赖者重链；库 `.h` → 依赖者经 depfile 重编
- [x] **2.7 i18n**：`workspace_*` key（X-macro 三向一致，`check_i18n.py` 通过）

### dev.3 — 测试与 CI（1.3.0-dev.3）✅

- [x] **3.1 集成测试**：3 成员 workspace（1 库 + 2 可执行，带 `[depends] workspace`）→ list / build -j / test / 依赖构建顺序 / 跨成员增量（库 .cpp→重链、库 .h→重编）/ clean / 失败汇总 + --stop-on-error（停派发 + skipped 计数）/ --member 含闭包 / **成员内独立构建（注入已存在产物、不触发闭包）** / **注入零环境变量** / 校验拒绝（路径逃逸、环、非 static 被依赖、嵌套、成员缺失）
- [x] **3.2 单测补全**：`resolve_member_ref`（完整路径/末段/未知/歧义）+ `resolve_jobs` 抽取（`-j` 显式 > `default_jobs` > auto）+ 自发现注入解析（存在性门控、无 `EZK_WS_*`）+ 响应文件阈值（含空格路径，dev.2 已覆盖）
- [x] **3.3 CI**：ubuntu job 追加 workspace 冒烟步骤（含依赖成员 + 二次构建增量断言）
- [x] **3.4 全量回归**：`bash build.sh test-all` 零失败（基线 853/4229，dev.3 后）

### dev.4 — i18n 语言变体（1.3.0-dev.4）✅

- [x] **4.1 标签归一化**：`normalize_locale_tag()`（`zh_CN`/`zh-CN`/`zh_CN.UTF-8`/大小写 → 规范 `zh-TW` 形）+ 单测
- [x] **4.2 检测路径**：`detect_language()` 重写——EZMK_LANG / Windows / POSIX 三路统一走 normalize + 有数据守卫（标签或首段）
- [x] **4.3 继承加载**：`init()` 基础语言 + 变体 overlay（`parse_locale_json` 覆盖模式）；回退链（变体缺失 → 基础 → 英文兜底）
- [x] **4.4 变体文件**：`locale/zh-TW.json`（繁体中文转写，继承式：只写差异键）
- [x] **4.5 校验扩展**：`check_i18n.py` 变体规则——子集合法（缺键=继承）/ 多余键报错（防漂移）/ meta.extends 校验；通过
- [x] **4.6 单测**（`test/test_i18n.cpp` 扩展）：归一化 + 回退链 + 继承合并 + 变体缺失 + detect 全路径；`bash build.sh test` 零失败
- [x] **4.7 收口**：全量回归（dev.3 基线 853/4229 → 861/4990，零回归）

### dev.5 — 消费命令总是自动构建（1.3.0-dev.5）✅

- [x] **5.1 test 总是构建**：`run_tests()` 移除 `check_built()`/`project_built` 存在性门控，无条件 `build_project`（utils 例外保留）
- [x] **5.2 pack --precompiled 总是构建**：`pack_project()` 移除存在性门控，无条件 `build_project` + 产物复核保留
- [x] **5.3 回归用例锁定**：改源码 → 直接 `ezmk test` / `pack --precompiled` 用新产物（断言编译发生/产物 hash 变化）；产物新鲜时秒过不重编
- [x] **5.4 文档**：CHANGES.md 行为变更条目（test/pack 现在总是增量构建）
- [x] **5.5 收口**：全量回归（dev.4 基线 861/4990 → 863/5007，零回归）

### pre.1 — 文档与发布收口（1.3.0-pre.1）✅

> 详细设计：[**1.3.0-pre.1.md**](plans/1.3.x/1.3.0-pre.1.md)（用户触达打磨 + 全量文档检查 + 发布门槛预核对）。

- [x] **6.1 文档**：cli.md（workspace 节，zh/en 中文基准先行）/ README 命令速览 / 教程新章（dev/05-workspace）/ CHANGES.md 1.3.0 聚合条目 + API 稳定性承诺扩展；cli.md `EZMK_LANG` 说明更新（变体标签 + 回退链）；config_file.md `[depends] workspace` 字段
