...
- [ ] **3.3 钩子处理**（设计 §3.1 行为约束）：`[hooks]` 不映射——生成注释块列出 `pre_build`/`post_build`/`on_failure` 并说明「EazyMake 沙箱 Lua，CMake 无等价物」；配置了钩子则 `message(WARNING)` 提示 CMake 构建不执行钩子后处理
- [ ] 阶段三自测：覆盖拒绝/`--overwrite`/`@link:` 解析/hooks 注释 + WARNING 用例通过
