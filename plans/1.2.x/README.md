...
1.2.2 为 **1.2.x 稳定线的文档补丁**：教程 14 章平铺无分类，改为移入分类子目录（`basic/` 入门 · `packages/` 包管理 · `dev/` 开发体验 · `interop/` 工具链互操作），README 索引分组展示；文件名编号保留、只改相对路径前缀，全部既有链接同步更新（README 高级特性表 / migrate-from-cmake / 教程内部交叉引用）。**纯文档变更，无代码/API 影响**。

| 版本 | 主题 | 关键交付 | 状态 |
|------|------|----------|------|
| [1.2.2](1.2.2.md) | 教程分类重组（子目录迁移） | `tutorial/zh|en/` 四分类子目录（basic/packages/dev/interop）+ README 分组索引 + 全仓链接修复（grep 零死链验收） | 待实现 |
