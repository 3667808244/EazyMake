...
> 1.2.x 系列 = **1.2.0 正式版**（dev.1 ~ dev.12 + pre.1/pre.2，2026-08-17 发布 ✅）+ **1.2.1 补丁**（按项目类型差异化模板生成，✅ 2026-08-18 发布，tag `v1.2.1`）+ **1.2.2 补丁**（教程分类重组，✅ 2026-08-18 完成）+ **1.2.3 补丁**（`ezmk example` 命令组，✅ 2026-08-18 完成）+ **1.2.4 补丁**（仓库文件夹包支持，✅ 2026-08-18 完成）+ **1.2.5 补丁**（测试缓存签名修复 01807bb + 默认包格式改为源码包，✅ 2026-08-19 开发完成，待发布）。
>
...
| 版本 | 主题 | 关键交付 | 状态 |
|------|------|----------|------|
| [1.2.5](1.2.5.md) | 测试缓存修复（01807bb）+ 默认包格式改为源码包 | test 缓存签名校验（Catch2 + ezmk 框架两路径）+ embed 剪枝；pack 默认源码包 + `--precompiled` 保留旧行为；pack 源码包端到端测试 + 全量零回归（794/3769） | **开发完成**（2026-08-19，待发布） |
