...
> 1.2.x 系列 = **1.2.0 正式版**（dev.1 ~ dev.12 + pre.1/pre.2，2026-08-17 发布 ✅）+ **1.2.1 补丁**（按项目类型差异化模板生成，✅ 2026-08-17 完成）。
>
...
| 版本 | 主题 | 关键交付 | 状态 |
|------|------|----------|------|
| [1.2.1](1.2.1.md) | 按项目类型差异化模板生成 + 默认配置补全 | `project new` 按类型生成：executable → main.cpp；static/shared → `include/<name>.hpp` + `src/<name>.cpp` 库骨架；utils → 不变；namespace 净化（`-`/`.`/空格 → `_`）；默认模板追加注释 `[test]` 示例节 | **已完成**（2026-08-17，全量 785/3629 零回归） |
