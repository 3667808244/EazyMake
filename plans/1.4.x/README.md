...
> 1.4.x 系列 = **1.4.0 功能版本**（dev.1 调试配置生成 → dev.2 工具链能力表 + 校验严格化 → dev.3 编译协商 → dev.4 CMake 互操作补全 → dev.5 功能收口 → pre.1 发布前收口，规划中）。
>
...
- **回归基线**：全量 968 用例 / 5588 断言（1.4.0-dev.5 后实测，基线 959/5492），新增功能不得引入回归。
