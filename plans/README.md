...
> 仓库根目录的 `plan.md` 为当前执行计划。**1.2.x 补丁系列（1.2.1~1.2.5）已全部发布/完成**（1.2.5：测试缓存修复 01807bb + 默认包格式改为源码包，2026-08-19 发布，tag `v1.2.5`）；当前推进 **1.3.0**（Workspace 工作区，功能 minor，dev → pre 拆分）。下一个破坏性窗口 **2.0.0**。
...
- **1.2.5** —— 1.2.x 稳定线补丁（测试缓存签名修复 `01807bb` + `ezmk project pack` 默认包格式改为源码包，`--precompiled` 保留旧行为）✅ 已发布（2026-08-19，tag `v1.2.5`）
