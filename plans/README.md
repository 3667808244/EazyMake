...
> 仓库根目录的 `plan.md` 为当前执行计划（**1.1.2**，补丁进行中，发布门槛已预检通过；发布后恢复 **1.1.3** 继续补丁线，之后恢复 1.2.0）。

---

## 当前执行

- **[1.1.2](1.1.x/1.1.2.md)** — 安全加固 + 静默错误修复（`plan.md`，发布门槛已预检通过，待打 tag）
- **[1.1.3](1.1.x/1.1.3.md)** — 安全收敛二轮 + 健壮性收口（承接 1.1.2 发布后的下一补丁，规划中）
- **[1.2.0](1.2.0/README.md)** — 工具链互操作与开箱工程化（`ezmk project cc` / CMakeLists.txt 导出 / 默认模板 Profile），补丁线结束后恢复推进
