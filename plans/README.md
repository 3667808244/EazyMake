...
> 仓库根目录的 `plan.md` 为当前执行计划。**1.2.x 补丁系列（1.2.1~1.2.5）已全部发布/完成**（1.2.5：测试缓存修复 01807bb + 默认包格式改为源码包，2026-08-19 发布，tag `v1.2.5`）；**1.3.0 已发布**（Workspace 工作区 + i18n 语言变体 + 消费命令总是自动构建，2026-08-21，tag `v1.3.0`）；**1.3.1~1.3.4 规划中**（1.3.1 区间语言标准 + 标准兼容校验 / 1.3.2 单元测试机器可读报告 / 1.3.3 workspace 双字母命令简写 / 1.3.4 watch 重建后自动运行）。下一个破坏性窗口 **2.0.0**。
...
- **1.4.0** —— 工具链能力表（`max_supported_std`）+ 编译协商（语义 B/C）+ 标准校验严格化开关 + CMake `CXX_STANDARD` 导入映射（预留，文档未写）
...
| 1.3.x | 功能版本 | 1.3.0 Workspace 工作区（`ezmk workspace` 命令组 + `ezmk-workspace.toml`；批量 + 并行）+ i18n 语言变体（dev.4，`zh-TW` 继承式变体）+ 消费命令总是自动构建（dev.5）（**已发布**，2026-08-21，tag `v1.3.0`）；1.3.1 区间语言标准 + 标准兼容校验（规划中）；1.3.2 单元测试机器可读报告（规划中）；1.3.3 workspace 双字母命令简写（规划中）；1.3.4 watch 重建后自动运行（规划中） | [1.3.x/README.md](1.3.x/README.md) |
...
graph TD
    ...
    v140["1.4.0 工具链能力表 + 编译协商（预留，文档未写）"]
    v200["2.0.0 (未来，破坏性变更)"]
