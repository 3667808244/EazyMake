...
> 仓库根目录的 `plan.md` 为当前执行计划。**1.2.x 补丁系列（1.2.1~1.2.5）已全部发布/完成**（1.2.5：测试缓存修复 01807bb + 默认包格式改为源码包，2026-08-19 发布，tag `v1.2.5`）；**1.3.0 已发布**（Workspace 工作区 + i18n 语言变体 + 消费命令总是自动构建，2026-08-21，tag `v1.3.0`）；**1.3.1 / 1.3.2 已完成（实现收口，2026-08-24；发布走补丁流程）**；**1.3.3~1.3.5 规划中**（1.3.3 workspace 双字母命令简写 / 1.3.4 watch 重建后自动运行 / 1.3.5 pack 多格式输出）。下一个破坏性窗口 **2.0.0**。

---

## 当前执行

- **1.3.3** —— 补丁：workspace 双字母命令简写（`wl`/`wb`/`wt`/`wc` → `workspace list/build/test/clean`；[`plans/1.3.x/1.3.3.md`](1.3.x/1.3.3.md)）。与 1.3.4 / 1.3.5 相互独立，可并行。
- **1.3.4** —— 补丁：watch 重建后自动运行（`ezmk watch --run` / `-r`，每次成功重建后阻塞运行产物；[`plans/1.3.x/1.3.4.md`](1.3.x/1.3.4.md)）。与 1.3.3 / 1.3.5 相互独立，可并行。
- **1.3.5** —— 补丁：pack 多格式输出（`ezmk project pack --format zip|tar.gz` + `.sha256` 边车；[`plans/1.3.x/1.3.5.md`](1.3.x/1.3.5.md)）。与 1.3.3 / 1.3.4 相互独立，可并行。
...
- **[1.3.x](1.3.x/README.md)** — 功能版本系列：1.3.0 Workspace 工作区（`ezmk workspace` 命令组 + `ezmk-workspace.toml` + i18n 语言变体 + 消费命令总是自动构建）**2026-08-21 已发布 ✅**（tag `v1.3.0`）；1.3.1 区间语言标准 + 标准兼容校验 **2026-08-24 实现收口 ✅**；1.3.2 单元测试机器可读报告 **2026-08-24 实现收口 ✅**（发布走补丁流程）
