...
- **[1.3.x](1.3.x/README.md)** — 功能版本系列：1.3.0 Workspace 工作区（`ezmk workspace` 命令组 + `ezmk-workspace.toml` + i18n 语言变体 + 消费命令总是自动构建）**2026-08-21 已发布 ✅**（tag `v1.3.0`）；1.3.1 区间语言标准 + 标准兼容校验 **2026-08-24 实现收口 ✅**（tag `v1.3.1`）；1.3.2 单元测试机器可读报告 **2026-08-24 实现收口 ✅**（tag `v1.3.2`）；1.3.3 workspace 双字母命令简写 **2026-08-25 已发布 ✅**（tag `v1.3.3`，三渠道分发：winget PR #423828 / homebrew tap / pacman PKGBUILD）；1.3.4 watch 重建后自动运行 **2026-08-25 实现收口 ✅**
