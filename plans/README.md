...
> 仓库根目录的 `plan.md` 为当前执行计划。**1.2.x 补丁系列（1.2.1~1.2.4）已全部发布/完成，1.2.5（测试缓存修复 01807bb + 默认包格式改为源码包）2026-08-19 开发完成、待发布**；当前推进 **1.3.0**（Workspace 工作区，功能 minor，dev → pre 拆分）。下一个破坏性窗口 **2.0.0**。
...
- **1.2.5** —— 1.2.x 稳定线补丁（测试缓存签名修复 `01807bb` + `ezmk project pack` 默认包格式改为源码包，`--precompiled` 保留旧行为）
...
| 1.2.x | 正式版 + 补丁 | 1.2.0 工具链互操作与开箱工程化 + 1.2.1 模板差异化（库骨架 .hpp/.cpp）+ 1.2.2 教程分类重组 + 1.2.3 `ezmk example` 命令组与内置示例 + 1.2.4 仓库文件夹包支持 + 1.2.5 测试缓存修复与默认源码包（规划中） | [1.2.x/README.md](1.2.x/README.md) |
| 1.3.x | 功能版本 | 1.3.0 Workspace 工作区（`ezmk workspace` 命令组 + `ezmk-workspace.toml`；批量 + 并行） | [1.3.x/README.md](1.3.x/README.md) |
...
graph TD
    ...
    v11x["1.1.x 正式版 + 补丁系列"]
    v12x["1.2.x 正式版 + 补丁系列"]
    v13x["1.3.x 功能版本（1.3.0 Workspace）"]
    v200["2.0.0 (未来，破坏性变更)"]
