...
> 仓库根目录的 `plan.md` 为当前执行计划。**1.2.x 补丁系列（1.2.1~1.2.5）已全部发布/完成**（1.2.5：测试缓存修复 01807bb + 默认包格式改为源码包，2026-08-19 发布，tag `v1.2.5`）；**1.3.0 已发布**（Workspace 工作区 + i18n 语言变体 + 消费命令总是自动构建，2026-08-21，tag `v1.3.0`）。下一个破坏性窗口 **2.0.0**。

---

## 当前执行

（暂无——1.3.0 已发布；下一个功能版本规划中。）
...
| 1.3.x | 功能版本 | 1.3.0 Workspace 工作区（`ezmk workspace` 命令组 + `ezmk-workspace.toml`；批量 + 并行）+ i18n 语言变体（dev.4，`zh-TW` 继承式变体）+ 消费命令总是自动构建（dev.5）（**已发布**，2026-08-21，tag `v1.3.0`） | [1.3.x/README.md](1.3.x/README.md) |
