...
> 1.1.x 系列 = **1.1.0 正式版**（dev.1 ~ dev.7 + pre.1 ~ pre.3，2026-08-07 发布 ✅）+ **1.1.1 补丁**（拦截 `ezmk utils cc` + 自动生成，2026-08-08 发布 ✅）+ **1.1.2 补丁**（安全加固 + 静默错误修复，发布门槛已预检通过）+ **1.1.3 补丁**（安全收敛二轮 + 健壮性收口，规划中）。
>
> 本目录为 1.1.x 全部文档的**平铺结构**（无子文件夹）：`1.1.0*.md` 为正式版系列，`1.1.1.md` / `1.1.2.md` / `1.1.3.md` 为补丁。
...
## 1.1.3 — 补丁（安全收敛二轮 + 健壮性收口）

1.1.3 为 **1.1.x 稳定线的补丁发布**（承接 1.1.2 发布后）：安全收敛二轮与健壮性收口。基于 2026-08-09 第二轮多模块安全与质量审计，修复 5 处安全缺口（钩子沙箱仍裸 `_G`、包名路径穿越、URL 直装无完整性、POSIX flags 命令注入、编辑器命令注入）与 5 处健壮性问题（`SOURCE_DATE_EPOCH` 崩线程、`~` 前缀截断、OVERLAPPED 全局池悬垂、`recursive_` 死代码、watcher/缓存边缘），并收敛 1.1.2 计划「延后项」中的手写 JSON 解析器与测试永真断言。**不新增命令、不弃用、不触碰 1.1.0 稳定 API**。

| 版本 | 主题 | 关键交付 | 状态 |
|------|------|----------|------|
| [1.1.3](1.1.3.md) | 安全收敛二轮 + 健壮性收口 | 钩子沙箱受限全局表统一；包名校验；URL 安装完整性确认；`join_shell_args` 元字符补全；编辑器命令转义；SDE 安全解析；`~` 前缀边界；OVERLAPPED 池实例化；`recursive_` 收口；`load_links_json` 改 nlohmann；测试永真断言清理 | 规划中 |

### 与 1.2.0 的关系

- 1.1.3 **先行**：钩子沙箱统一与权限门控是安装链路的执行边界收敛；`config`/`cache` 重构（Q1/Q2）是 1.2.0 复用点的稳定性加固。
- 1.2.0 承接：功能计划（`ezmk project cc` / CMake 导出 / 模板 Profile）不受影响；本版「延后项」（watcher 静默死亡/kqueue 缺口、宽窄字符路径、i18n 二次替换、S1b 若收口）归 1.2.0 或后续补丁。
