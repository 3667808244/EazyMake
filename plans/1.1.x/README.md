...
> 1.1.x 系列 = **1.1.0 正式版**（dev.1 ~ dev.7 + pre.1 ~ pre.3，2026-08-07 发布 ✅）+ **1.1.1 补丁**（拦截 `ezmk utils cc` + 自动生成，2026-08-08 发布 ✅）+ **1.1.3 补丁**（安全收敛二轮 + 健壮性收口，2026-08-10 发布 ✅；承接 1.1.2 的未单独发布修复，安全加固 + 静默错误修复并入本版）。
>
...
| 版本 | 主题 | 关键交付 | 状态 |
|------|------|----------|------|
| [1.1.3](1.1.3.md) | 安全收敛二轮 + 健壮性收口 | 钩子沙箱受限全局表统一；包名校验；URL 安装完整性确认；`join_shell_args` 元字符补全；编辑器命令转义；SDE 安全解析；`~` 前缀边界；OVERLAPPED 池实例化；`recursive_` 收口；`load_links_json` 改 nlohmann；测试永真断言清理 | 已发布 ✅ |
