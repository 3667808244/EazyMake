...
| [1.2.0-dev.9](1.2.0-dev.9.md) | 包构建配置收敛（dev.7 延伸） | 包 `[compile].src_dirs` / `include_dirs` 生效：`collect_sources` 复用 + include 去重 + utils 门控对齐 + `pkg info` 增显 | 待实现 |
...
> **执行计划**：根 `plan.md` 为当前执行计划，镜像正在推进的子版本设计文档（dev.1 ~ dev.7 已完成，下一个子版本为 [1.2.0-dev.8](1.2.0-dev.8.md)，dev.9 紧随 dev.8；dev.10 紧随 dev.9，dev.11 依赖 dev.10，pre.2 为 dev.10 之后的文档化检查点）。
