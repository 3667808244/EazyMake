...
> **执行计划**：根 `plan.md` 为当前执行计划，镜像正在推进的子版本设计文档（dev.1 ~ dev.9 已完成、dev.12 已完成，**当前执行 [1.2.0-dev.10](1.2.0-dev.10.md)**，dev.11 依赖 dev.10，pre.2 为 dev.10 之后的文档化检查点）。
...
- **回归基线**：当前全量测试 727 用例 / 3361 断言（dev.12 后基线），重构与新增命令均不得引入回归。
