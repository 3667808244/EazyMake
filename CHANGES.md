...
- **winget**：split manifests（`InstallerType: zip` + `NestedInstallerType: portable`，`InstallerSha256` = `32012a27…`）已提交 `microsoft/winget-pkgs#437604`；本机 `winget validate` 通过（「清单验证成功」），`license/cla` 已 pass；PR 的 **8 项 Azure 校验全部通过**（含 `Installation Validation` 32m30s），**已获版主批准并合并**（标签 `Moderator-Approved` + `Publish-Pipeline-Succeeded`，上游 `manifests/e/EazyMake/EazyMake/1.4.3/` 三文件已在 master）——`winget install EazyMake.EazyMake` 自此安装 1.4.3（1.4.2 的 `#434086` 亦已合并）
...
- **三渠道收口**：Release 资产（含 `man/`）→ Homebrew tap / pacman 出包 / winget 上线 **全部完成，无遗留跟进项**
- **文档补记（发布后）**：`README.md` / `README_ZH.md` 与 `docs/{en,zh}/cli.md` 的安装节补 **winget** 渠道（此前只在开发向的 `ezmk-publish` skill 中记录），使用户文档与三条分发渠道一致
...
### 发布（2026-08-26，tag `v1.3.6`）

- **GitHub Release**：windows-x64 zip + linux-x64 / macos-arm64 tar.gz + 独立 `ezmk.exe`/`ezmk-lua.exe`（含 `.sha256` 边车）已上传（`macos-x64` 仍无资产——`macos-13` runner 在 free tier 不分配，与 1.2.x/1.3.0/1.3.3 相同）
- **winget**：split manifests 已提交 `microsoft/winget-pkgs#424481`（`InstallerSha256` 取 Release 资产真实 digest `0d677b9a…`；`license/cla` 已 pass，CI 校验进行中，版主审批为发布后跟进项，不阻塞发布）
- **Homebrew**：tap `3667808244/homebrew-eazymake` 公式已更新至 1.3.6（macos-arm64 `c889c5ee…` / linux-x64 `a98f5b75…` 真实 digest；仓库副本 `publish/homebrew/ezmk.rb` 同步）
- **pacman**：`publish/arch/PKGBUILD` `pkgver=1.3.6` + 源码 tarball 真实 digest `84d06d6a…`（2026-08-26 实测下载）；AUR 仍延后
