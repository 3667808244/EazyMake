...
> **为什么以包的形式分发工具？** 工具作为普通包分发，新增工具无需重新编译或重新发布 `ezmk`——任何人都可以分发工具，运行时通过作用域链自动发现。

> **已弃用工具：** `ezmk utils cc` 自 1.2.0 起弃用——请使用内置命令 `ezmk project cc`。该工具保留可用至 2.0.0；运行 `ezmk utils cc` 会输出弃用提示。
...
> **为什么把写入限制在项目根目录内？** 工具只应改动它当前操作的项目；没有这个限制，脚本可能覆盖主机上任意文件。把写入限制在项目根目录内，让工具默认就是安全的。
...
`ezmk.api_version` 是一个整数，仅在**不向后兼容的变更**时递增。脚本可以检查它来做兼容性判断：
...
| 变更类型 | 版本号递增？ | 说明 |
|---|---|---|
| 新增函数 | **否** | 向后兼容 — 旧脚本不会调用它 |
| 为已有函数增加可选参数 | **否** | 已有调用方式不受影响 |
| 废弃函数（`@deprecated since v<N>`） | **否** | 函数仍可用；保留至少 2 个 minor 版本后移除 |
| 移除已废弃的函数 | **是** | 破坏性变更 — 使用该函数的脚本将失败 |
| 修改函数的必选参数数量或类型 | **是** | 破坏性变更 |
| 修改函数返回值的语义 | **是** | 破坏性变更 |

### 当前版本

**`ezmk.api_version = 1`**（在 ezmk 0.9.4 中引入）。

- 版本号递增时，旧 API 接口将至少保留 2 个 minor 版本，并在 Lua 中输出废弃警告（`ezmk.warn(...)`）。
- 废弃的 API 将在本文档中明确标注 `@deprecated since v<N>` 标记。

> **为什么需要显式的 API 版本号？** 这样脚本能判断当前 `ezmk` 提供哪些能力，并在升级后继续工作——`ezmk.api_version` 仅在破坏性变更时递增，`>= 2` 这类检查是可靠的兼容性信号，旧脚本在旧版 ezmk 上仍能运行。
...
> **为什么 deny 优先？** 白名单开得过宽是常见的失误，因此 deny 必须永远优先——即使同一目标也被列入白名单。未列出的访问则落入交互式询问，而不是静默放行。
...
> **为什么 CI 中默认拒绝？** 没有人在终端前批准未知访问时，安全的默认是拒绝并打印被拦截的目标——自动化应事先在白名单中声明所需访问，而不是静默获得宽泛权限。
...
> **为什么旧包保持无限制？** 这样现有 utils 包无需立刻更新即可继续工作；一次性 warning 提示维护者按自己的节奏补充权限声明。
