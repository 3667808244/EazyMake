...
- **为什么不设计**：
  - **结构性复杂度**——EazyMake 已有**数据**：per-file 编译耗时（1.2.0-dev.6：`-v` 全量排序明细 / 慢构建自动 top-N，见 `src/build.cpp` 的 `compile_phase` 计时逻辑）。但火焰图需要**结构化时序导出 + 交互式渲染层**（HTML/JS/SVG 资产 + 悬浮/缩放交互）——后者是全新子系统，不是对现有统计的增量。
