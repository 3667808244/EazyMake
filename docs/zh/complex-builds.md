...
以下四种形态会把构建推出 EazyMake 的适用范围——其中第四种只是部分越界，工作区（1.3.0）已覆盖它的常见形态。命中任意一种里尚未支持的部分，就该改用 CMake。
...
- **为什么超出范围**：EazyMake 的模型是一个 `ezmk.toml` 一个产物——没有 `--target` 从多个产物里挑一个，也没有 per-target 配置。跨**独立项目**的单向非循环依赖图由工作区支持（见第 4 条）；仍超出范围的是单项目内多产物、per-target 配置和成环的依赖图。
- **替代方案**：拆成每个产物一个项目，依赖图单向非循环时用工作区把它们连起来；成环时用 CMake 的 `add_executable` / `add_library` 图。（`utils` 包可以靠 `[utils].tools` 内置多个工具，同一产物的多形态用 `[compile.profile.*]`——这些不算多目标。）
...
- **1.3.0 起已支持（最常见的 monorepo 形态）**：`ezmk-workspace.toml` 定义成员集合（`[workspace] members`），成员可用 `[depends] workspace = [...]` 声明对其他成员的**单向非循环**依赖。`ezmk workspace list / build / test / watch / clean / scan` 统一驱动整套成员；构建按拓扑排序，`static` 成员的产物会被自动复用（`-I <ws>/<m>/include` + `-L <ws>/<m>/build -l<m>` 自动注入）。确切边界见 [non-goals.md](non-goals.md)。
- **仍属于"复杂构建"的部分**：per-target 配置与从多个产物中选一个（`--target`）、依赖**成环**、依赖类型非 `static` 的成员、依赖**版本约束** / 快照语义（这些走包），以及把构建写成**可编程的图**。
- **替代方案**：每个产物一个项目，用成员依赖图单向非循环的工作区把它们连起来；需要版本化复用时改用独立项目 + 共享包；需要成环、per-target 选择或可编程构建图时用 CMake。
...
| | 否（交叉编译） | → CMake |
| 我的项目之间是**单向且不成环**的依赖？ | 是（工作区 `[depends] workspace`） | EazyMake ✓ |
| | 否（依赖成环，或单项目多产物） | → CMake |
...
- 你需要让一个项目链接另一个成员的**动态库**或可执行文件（只有 `static` 成员可被依赖）、让一个项目产出多个产物，或让成员之间**相互成环**依赖。
