...
声明**最低兼容标准**——适合库作者（代码以某个基线标准为下限，可选地声明已知可工作的上界）：

| 用户输入 | min | max | 生效 `-std=` | 含义 |
|----------|-----|-----|-------------|------|
| `C++11` | 11 | — | `-std=c++11` | 精确标准（行为不变） |
| `>=C++11` | 11 | — | `-std=c++11` | 至少 C++11 |
| `C++11..C++17` | 11 | 17 | `-std=c++11` | C++11 最低、C++17 为上界（元数据） |
| `>=GNUCPP11` | 11 | — | `-std=gnu++11` | 至少 GNU C++11 |
| `>=C` | 11 | — | `-std=c11` | 至少 C11（无版本号时按默认值） |

- **最小值**是*生效标准*：编译始终用 `min`，产物行为最可预测、兼容性最大。
- **最大值**（仅双边区间）是**元数据**：声明项目已知可工作的上限，不改变编译标志，也不进入缓存签名（只改 max 不触发重建）。
- `EZMK_LANG` 取 min 规范形（`">=C++11"` → `"CPP11"`），与精确写法完全一致，既有 `#ifdef` 代码不受影响。
- 非法形式会被拒绝：`C++17..C++11`（max < min）、`>C++11`（只支持 `>=`）、`C++11+`（不支持 `+` 后缀）、区间端为空（`C++11..` / `..C++17`）、`>=C++11..C++17`、跨语言族区间（`C11..C++17`）。

> **何时用区间？** 包作者应声明代码能编译的*最低*标准（如基线 C++11、新标准可选时写 `">=C++11"`）。自 1.3.1 起，安装期的标准兼容校验会在包的最低要求高于消费者项目编译标准时给出警告——见 [包编写指南](package_authoring.md) 与 [包管理](pkg.md)。
>
> **编译协商（1.4.0-dev.3+）：** *源码包*的生效编译标准为 `min( max(包min, 消费者min), max_supported_std, 包max )`——消费者标准更高时使用之，前提是编译器支持（能力表）且包声明允许（区间 **max** 是承诺边界：消费者再高也不会把包推到上界之上）。完整语义见 [包编写指南](package_authoring.md)。
>
> **CMake 导入（1.4.0-dev.4+）：** `ezmk project import` 把 `CXX_STANDARD N` / `cxx_std_N` 映射为 `">=CPP<N>"`（单边区间）——与 CMake "至少 N" 语义一致，区间形式是天然目标。见 [cli.md](cli.md)。
...
| `EZMK_VERSION` | 字符串 | `"1.4.4"` | EazyMake 自身版本号 |
...
| `workspace` | string[] | 否 | `[]` | **1.3.0+** 工作区兄弟成员依赖（开发中源码即改即用） |

> **为什么缺失时只警告而不是失败？** `want` 表达的是可选依赖。缺失时定义 `EZMK_LIB_MISS_<NAME>` 宏，代码可据此 `#ifdef` 绕过该特性、优雅降级，而不是中断构建。

### 工作区兄弟依赖（`workspace`，1.3.0+）

`workspace` 声明对**同一工作区**内其他成员（兄弟项目）的依赖——开发中源码即改即用，构建时自动注入兄弟的静态库产物：

```toml
[depends]
workspace = ["strutil"]          # 兄弟成员（末段或完整相对路径，如 "libs/strutil"）
```

- **前置条件**：项目必须是某工作区（`ezmk-workspace.toml`）的成员；工作区声明与定位见 [`cli.md`](cli.md) 的 `workspace` 节。
- **约束**：依赖图必须**单向非循环**（环 / 自环在配置期拒绝）；被依赖成员必须是 `type = "static"`（产物 `build/lib<name>.a` 复用）；无版本（版本/快照语义走包，见上方 `lib`）。
- **注入**：构建该成员时自动注入 `-I <ws>/<m>/include` + `-L <ws>/<m>/build -l<m>`（存在性门控、零环境变量）；改库 `.cpp` → 依赖者只重链接，改库 `.h` → 依赖者自动重编。
- **非成员项目声明**了 `workspace` 但向上找不到工作区 → 构建时警告并跳过注入（不阻断）。
- 完整命令（`workspace build / test / clean`、`-w` 重定向、`--member`）见 [`cli.md`](cli.md)。
...
包管理配置。可选——所有字段默认保持「信息性（不失败）」行为。

| 字段 | 类型 | 必须 | 默认值 | 说明 |
|------|------|------|--------|------|
| `strict_std_check` | bool | 否 | `false` | **1.4.0-dev.2+** 把安装期标准兼容校验（1.3.1）从警告升级为**致命错误**：当包的最低标准超过本项目的 `[project].language` 时直接失败 |

> **何时开启？** 设置 `strict_std_check = true` 后，安装最低标准高于本项目编译标准的包会**失败**
> （而不是警告后继续）。用于 CI 或严格要求的环境，让 ABI/源码不匹配直接阻断安装。
> 解决失败：提高 `[project].language`——或设回 `strict_std_check = false` 降级为 1.3.1 的警告。
> 默认保持警告是因为严格失败会破坏现有包生态（仅显式开启）。

```toml
[pkg]
strict_std_check = true   # warn → fatal（默认 false）
```
