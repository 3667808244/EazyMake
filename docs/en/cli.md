...
- **Skip rules:** hidden entries (`.` prefix, e.g. `.git`/`.ezmk`) are **skipped silently**; **nested workspace roots** (the whole subtree containing its own `ezmk-workspace.toml`) and directories escaping the root via symlink are **printed with their skip reason**. The scan root itself is never a member, even when it has its own `ezmk.toml`.
...
| `EZMK_NO_MAN` | install | Set to `1` to skip manual page install (`install.sh`) |
