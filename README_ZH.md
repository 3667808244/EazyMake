...
可通过 `PREFIX`、`EZMK_REF`、`EZMK_VERSION`、`EZMK_NO_DEFAULT_REPO`、`EZMK_NO_COMPLETIONS`、`EZMK_NO_MAN` 自定义。详见[安装选项](#安装选项)。在 Linux / macOS / MSYS2 上安装脚本会一并安装手册页，`man ezmk` 与 `man 5 ezmk.toml` 可离线查阅。
...
| `-Version`（PS） | 要安装的版本标签——**必须带 `v` 前缀**，如 `"v1.4.4"` | `"latest"` |
...
| `ezmk.lock` 确定性构建 | 锁定依赖版本与内容哈希，`--locked` 下 CI 可复现构建 | [`docs/zh/config_file.md`](docs/zh/config_file.md) · 教程 [包 02](tutorial/zh/packages/02-version-lockfile.md) |
