    ...
    if (action == "add") {
        ...
        for (int i = 3; i < argc; ++i) {
            ...
            if (parse_scope_flags(argv[i], parsed, true)) {
                ...
                if (i + 1 >= argc) util::fatal("'--name' requires a value");
                opts.name = argv[++i];
            } else if (std::strcmp(argv[i], "--branch") == 0) {
                if (i + 1 >= argc) util::fatal("'--branch' requires a value");
                opts.branch = argv[++i];
            } else {
                if (has_url) util::fatal("'ezmk repo add' takes only one URL or path");
                opts.url = argv[i];
                has_url = true;
            }
        }

        if (!has_url) util::fatal("'ezmk repo add' requires a git URL or local path");
        if (!scope_set) opts.scopes = {Scope::Project};
    ...
    if (action == "list") {
        ...
        for (int i = 3; i < argc; ++i) {
            ...
            if (parse_scope_flags(argv[i], parsed, false)) {
                for (auto s : parsed) opts.scopes.push_back(s);
                scope_set = true;
            } else {
        ...
        if (!scope_set) opts.scopes = {Scope::Project, Scope::User, Scope::Global};
        args.repo_opts = std::move(opts);
        return args;
    }
...
CliArgs parse(int argc, char** argv) {
    ...
    if (group == "project") return parse_project_args(argc, argv);
    if (group == "pkg")     return parse_pkg_args(argc, argv);
    if (group == "repo")    return parse_repo_args(argc, argv);
